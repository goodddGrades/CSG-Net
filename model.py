...
class BiSGTAR(nn.Module):
    def __init__(self, args):
        ...
        self.cl_proj_dim = getattr(args, 'cl_proj_dim', 0)
        if self.cl_proj_dim and self.cl_proj_dim > 0:
            self.proj_rna = nn.Linear(args.hidden, self.cl_proj_dim)
            self.proj_dis = nn.Linear(args.hidden, self.cl_proj_dim)
        else:
            self.proj_rna = None
            self.proj_dis = None
    ...
    def forward(self, feat):
        ...
        yc, hc, zc = self.AE_rna(rna_sparse_feat)
        yd, hd, zd = self.AE_dis(dis_sparse_feat)

        zc_proj = self.proj_rna(zc) if self.proj_rna is not None else None
        zd_proj = self.proj_dis(zd) if self.proj_dis is not None else None

        return yc, rna_sparse_feat, rna_quality, hc, yd, dis_sparse_feat, dis_quality, hd, zc, zc_proj, zd, zd_proj
